...
int main(int argc, char **argv) {
      ...
      case 0:
        switch (option_index) {
          ...
          case 4:
            timeout = atoi(optarg);
            if (timeout < 0){
              printf("Timeout: %d cannot be negative\n", timeout);
              return -1;
            }
            break;
          defalut:
  ...
  if(with_files) f_min_max = fopen("/workspaces/os_lab_2019/lab3/src/f_min_max.txt", "w");
  pid_t* arr_pid = malloc(sizeof(pid_t) * pnum);//for lab4
